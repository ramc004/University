...
\begin{document}
    ...
    \addcontentsline{toc}{subparagraph}{\ac{AI}\_Based\_Smart\_Bulb\_for\_Adaptive\_Home\_\\AutomationApp.swift}
    ...
    \subparagraph*{Network Configuration and ngrok Integration}\mbox{}\\
    \addcontentsline{toc}{subparagraph}{Network Configuration and ngrok Integration}
    To enable the iOS app to communicate with the Flask backend server from any network location, ngrok was integrated to expose the local development server running on port 5000 to the public internet. An ngrok account was created at \href{https://ngrok.com}{ngrok.com}, and the ngrok agent was configured to create a secure tunnel forwarding \ac{HTTP} traffic from a public \ac{URL} to localhost:5000. This removed the requirement for the development machine and iOS device to be on the same local network, enabling flexible testing from any location.

    To streamline the development workflow, a custom terminal alias was created to launch both the Flask server and the ngrok tunnel in separate terminal windows using a single command. This ensured the backend server was immediately accessible via a public ngrok \ac{URL}, which could then be updated in APIConfig.swift for the iOS app to connect. The ngrok dashboard provided real time monitoring and inspection of \ac{HTTP} requests and responses, which proved valuable during debugging and \ac{API} development. This setup enabled seamless testing of authentication, bulb management, and real time device control on physical iOS devices without network constraints.
