...
struct RegisterPasswordView: View {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading) {
                    ...
                    if !codeInput.isEmpty && !codeExpired {
                        Text(codeValid ? "Code correct" : "Code incorrect").foregroundColor(codeValid ? .green : .red).bold()
                    }
                ...
                VStack(alignment: .leading) {
                    ...
                    HStack {
                        if showPassword { TextField("Enter password", text: $password) }
                        else { SecureField("Enter password", text: $password) }
    ...
    func registerUser() {
        ...
        request.httpBody = try? JSONSerialization.data(withJSONObject: ["email": email, "password": password] as [String: Any], options: [])
            ...
            DispatchQueue.main.async {
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    if httpResponse.statusCode == 200 {
                        // Store email locally for session management only
